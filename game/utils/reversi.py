...
class Reversi:
    ...
    def reset_score(self):
        for player in self.board.wins:
            self.board.wins[player] = 0
    ...
    def play(self):
        ...
        # Create the reset score button
        rest_score_button_size = (self.setup.screen_width * .055, self.setup.screen_height * .1)
        rest_score_button_position = ((self.setup.screen_width - rest_score_button_size[0])* .92, (self.setup.screen_height - mute_button_size[1]) * .03)
        self.rest_score_button = CircleButton(rest_score_button_position, rest_score_button_size)
        ...
        # Player blinking state
        self.player_blink = False
        ...
        while running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    elif self.rest_score_button.is_pressed(event.pos):
                        if self.state is GAME_STATE.OVERLAY:
                            self.reset_score()
            ...
            self.rest_score_button.update_status(mouse_pos)
            self.rest_score_button.draw(self.setup.screen)
